...
def solve_adaptive_rk43_procedure(f, y0, I, h0, epsilon, q):
	"""Adaptive Runge–Kutta 4(3) integrator.

	Embedded pair: 4th order weights b = (1/6, 1/3, 1/3, 1/6)
	Lower (embedded) 3rd order weights b* = (1/6, 2/3, 0, 1/6)

	Parameters
	---------
	f : callable
		Right–hand side f(t, y) or f(y) returning array_like.
	y0 : array_like
		Initial value.
	I : tuple (a, b)
	...
	h0 : float
		Initial step size.
	epsilon : float
		Tolerance for infinity–norm local error estimate.
	q : int
		Order of the lower method (for RK4(3) use q=3).
	...
	y_values : np.ndarray
		Solution values at accepted time points.
	fevals : int
	...
	while t < b:
		if t + h > b:
			h = b - t  # Adjust last step to end exactly at b
		...
		# Compute 4th and 3rd order estimates
		...
		if s >= 1.0:
			...
			# rad et, h ni kichraytir va davom et (qayta urinish)
			h = max(0.5, s) * h
			continue
...
if __name__ == "__main__":
	...
	h0 = 1e-4
	epsilon = 1e-4
	q = 3  # Order of the lower method in RK4(3)
